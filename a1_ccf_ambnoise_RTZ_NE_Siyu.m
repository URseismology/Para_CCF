...
% Nworkers = 4; % number of workers in pool for parallel processing
 delete(gcp('nocreate')); % @Siyu comment for debug
% parpool(Nworkers);
...
for ista1=1%:nsta
    ...
    for ista2=1:nsta
        ...
        nFile = floor(length(list1));
        for ifil = 1:nFile % Can manipulate this number during the debug section 
            file1cZ = list1(ifil).name;
            file1cH1 = strrep(file1cZ,[comp,'Z'],[comp,'N']); % switch N to 1 if using LH1 instead of LHN
            file1cH2 = strrep(file1cZ,[comp,'Z'],[comp,'E']); % switch E to 2 if using LH2 instead of LHE
            ...
            data1cH1=dir([datadir,sta1,'/',year,'/',sta1,'.',hdayid,'.*N.sac']);
            data1cH2=dir([datadir,sta1,'/',year,'/',sta1,'.',hdayid,'.*E.sac']);
            data1cZ= dir([datadir,sta1,'/',year,'/',sta1,'.',hdayid,'.*Z.sac']);
            data2cH1=dir([datadir,sta2,'/',year,'/',sta2,'.',hdayid,'.*N.sac']);
            data2cH2=dir([datadir,sta2,'/',year,'/',sta2,'.',hdayid,'.*E.sac']);
...
%             tic
            %parfor iwin = 1:nwin % @ Siyu -- remove during debug
            for iwin = 1: nwin
...
%                 set(gcf,'position',[400 400 600 300]);
            ...
            if IsSaveTxt % @ Siyu: save the results in a txt file
                ...
                % List 2: sample rate, # of days, snr acausal, snr
                % causal, # of points in frequency domain
                ...
                textpath_Z = ['./text_output/RayleighResponse/','dispersion_',sta1, '_', sta2, '.txt'];
                text_Z = fopen(textpath_Z, 'w');
                fprintf(text_Z, '%6.5f %6.5f %6.5f %6.5f %6.5f\n', list1);
                fprintf(text_Z, '%6.5f %6.5f %6.5f %6.5f %6.5f\n', list2);
                fprintf(text_Z, '%6.5f %6.5f %6.5f %6.5f %6.5f\n', Z); 
                fclose(text_Z);
                % T: frequency, real(ccf), imag(ccf), real(ccf), imag(ccf)
                T = [faxis(ind); real(coh_sumT(ind)/coh_num); imag(coh_sumT(ind)/coh_num);
                    real(coh_sumT(ind)/coh_num); imag(coh_sumT(ind)/coh_num)];
                % create txt file for Love Wave
                textpath_T = ['./text_output/LoveResponse/','dispersion_',sta1, '_', sta2, '.txt'];
